...
import { doc, getDoc, updateDoc } from './firebaseConfig.js'
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!userId) {
    ...
    window.location.href = "./profile.html"; // Redireciona para a página de perfil se não encontrar o userId
    return;
  }
  ...
  onAuthStateChanged(auth, async (user) => {
    if (!user || user.uid !== userId) {
      ...
      window.location.href = "../auth/login.html"; // Redireciona para o login se o usuário não estiver logado
      return;
    }
...
async function updateProfile(userId) {
  ...
  try {
    ...
    window.location.href = `./profile.html?userId=${userId}`; // Redireciona para a página de perfil após a atualização
  } catch (error) {
